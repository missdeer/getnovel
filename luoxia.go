...
func downloadLuoxiaPage(u string) (c []byte) {
	var err error
	c, err = httputil.GetBytes(u, httpHeadersLuoxia, time.Duration(opts.Timeout)*time.Second, opts.RetryCount)
	if err != nil {
		return
	}
...
func downloadLuoxia(u string, gen ebook.IBook) {
	b, err := httputil.GetBytes(u, httpHeadersLuoxia, time.Duration(opts.Timeout)*time.Second, opts.RetryCount)
	if err != nil {
		return
	}
	...
	dlutil := newDownloadUtil(downloadLuoxiaPage, gen)
	dlutil.process()
