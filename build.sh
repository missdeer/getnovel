...
if [ "$OS" == "Darwin" ]; then
	for lua in lua51 lua52 lua53 lua54 luajit; do
		env CGO_ENABLED=1 GOOS=darwin GOARCH=arm64 go clean --tags ${lua}
		...
		env CGO_ENABLED=1 GOOS=darwin GOARCH=amd64 go clean --tags ${lua}
	...
	for lua in lua51 lua52 lua53 lua54 luajit; do
		env CGO_ENABLED=1 go clean --tags ${lua}
