...
func preprocess69xinshuChapterListURL(u string) string {
	reg := regexp.MustCompile(`https://www\.69shuba\.com/book/([0-9]+)\.htm`)
	if reg.MatchString(u) {
		ss := reg.FindAllStringSubmatch(u, -1)
		s := ss[0]
		return fmt.Sprintf("https://www.69shuba.com/book/%s/", s[1])
	}
	return u
}
...
func init() {
	registerNovelSiteHandler(&config.NovelSiteHandler{
		Sites: []config.NovelSite{
			{
				Title: `69书吧`,
				Urls:  []string{`https://www.69shuba.com/`},
			},
		},
		CanHandle: func(u string) bool {
			patterns := []string{
				`https://www\.69shuba\.com/book/[0-9]+/`,
				`^https://www\.69shuba\.com/book/[0-9]+\.html?$`,
			}
