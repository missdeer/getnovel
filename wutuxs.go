...
var (
	httpHeadersWutuxs = http.Header{
		"Referer":                   []string{"http://www.wutuxs.com/"},
...
func downloadWutuxsPage(u string) (c []byte) {
	var err error
	c, err = httputil.GetBytes(u, httpHeadersWutuxs, time.Duration(opts.Timeout)*time.Second, opts.RetryCount)
	if err != nil {
		return
	}
...
func downloadWutuxs(u string, gen ebook.IBook) {
	b, err := httputil.GetBytes(u, httpHeadersWutuxs, time.Duration(opts.Timeout)*time.Second, opts.RetryCount)
	if err != nil {
		return
	}
	...
	dlutil := newDownloadUtil(downloadWutuxsPage, gen)
	dlutil.process()
