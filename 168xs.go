...
var (
	httpHeaders168xs = http.Header{
		"Referer":                   []string{"http://www.168xs.com/"},
...
func download168xsPage(u string) (c []byte) {
	var err error
	c, err = httputil.GetBytes(u, httpHeaders168xs, time.Duration(opts.Timeout)*time.Second, opts.RetryCount)
	if err != nil {
		return
	}
...
func download168xs(u string, gen ebook.IBook) {
	tocURL := u
	b, err := httputil.GetBytes(tocURL, httpHeaders168xs, time.Duration(opts.Timeout)*time.Second, opts.RetryCount)
	if err != nil {
		return
	}
	...
	dlutil := newDownloadUtil(download168xsPage, gen)
	dlutil.process()
